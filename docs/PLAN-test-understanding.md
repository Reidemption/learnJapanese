...
- **No retry-missed for tests.** A retry would test short-term memory of the answers you just saw. Instead the result screen offers "Practise the 7 you missed" (a practice session limited to those units) and, later, a word test of them.
...
- **Similarity ranking:** candidates are scored and the closest ones picked, with the seeded `rng` breaking ties:
  - meaning (EN choices): the same `note` (part of speech, such as `ru-verb`), a similar word count, and the same deck first
  - reverse (JA choices): a shared kanji, a similar kana length, and the same `note`
...
- A test session is an attempt with `mode: "test"` and a `scope` of `deck` or `words`. A word test spans decks, so its `deck_id` is empty. Per-deck results always come from mapping each answer's unit to its deck in TS, which works for both kinds.
...
- Server validation accepts `"test"` as an attempt mode, requires a known per-answer mode on test answers, and allows an empty `deckId` only for word tests.
...
- Routes:
  ...
  - the result screen lists each unit as pass/fail with the right answers and offers "Practise the ones you missed"
  - no retry
...
- [ ] Ranking tests with fixed fixtures: a same-`note` meaning candidate outranks a different one, and a shared-kanji reverse candidate outranks an unrelated one.
...
- `analytics.ts`:
  ...
  - `wordSet(kind, level, progress, limit = 20)` for the Ready / Missed / Weak sets, oldest-tested first
...
- Word tests:
  ...
  - recorded with `scope: "words"` and an empty `deckId`
- Dashboard:
...
- [ ] The Go server accepts a word-test attempt with an empty `deckId` and rejects an empty `deckId` on a deck test or practice session.
