...
- Tags on grammar or particles decks.
...
- Item-level tags cover what the deck doesn't:
  ...
  - words that belong to a tag outside its obvious deck, e.g. 行く in `n5-kanji-verbs` gets `verb`, and 今日 in `n5-kanji-numbers-days` gets `time`
...
`TAGS` lives in `src/types.ts` next to `DECK_GROUPS`. Each entry is `{ id, label, labelJa, facet }`, and `facet` is `"type"` or `"theme"`.

Starting list (Tags Phase 1 may adjust it, within the rules below):

| Facet | Tags |
|---|---|
| type | noun, number, counter, verb, u-verb, ru-verb, irregular-verb, i-adj, na-adj, adverb, question-word, connector, expression |
| theme | time, place, people, food, nature, body, home, school, transport |

**Rules, enforced by `src/content.test.ts`:**
- **Every tag spans ≥ 2 decks.** A tag that matches one deck is just a copy of that deck. For example, "animals" isn't a tag, because the Animals deck already is that set.
- **Every tag has ≥ 8 distinct words**, so a Custom deck made from it alone can build 4-choice questions.
- **Grammar and particles decks aren't tagged.** Their items are grammar forms and cloze questions, not words.

### Building a Custom deck (`src/study/tags.ts`)
Pure functions with a colocated `tags.test.ts`:

- `tagsOf(deck, item): Tag[]`: the deck's tags plus the item's.
- `matchingUnits(decks, level, selected)`: the words that match the selected tags.
  ...
  - **Duplicates are removed** by plain `ja` text (山 is in both `n5-kanji-nature` and `n5-nature-weather`). The copy kept is the one from the deck that comes first in `decks` order.
- `pickUnits(candidates, stats, rng, { size = 20, order = "needs-work" })`: caps a candidate list. The default `"needs-work"` order is:
  ...
  Ties are broken by `rng`, so "New set" gives a different pick. The order is a parameter because Test Phase 3's `wordSet` reuses this with an oldest-tested order.
...
- `src/types.ts`: add `TAGS`, `Tag`, `TAG_FACETS`, and `tags?` on `Deck` and `Item`.
- Tag all 50 decks: deck defaults first, then item-level additions.
...
- Checks added to `src/content.test.ts`:
  ...
  - no tags on grammar or particles decks, at deck or item level
  - no item repeats a tag its deck already declares
  - every tag spans ≥ 2 decks and ≥ 8 distinct words
  - `u-verb`, `ru-verb` and `irregular-verb` each imply `verb`
  - an item whose `note` is a verb class ("u-verb", "ru-verb", "irregular") has the matching tag
...
**A/C**
- [ ] The content checks above are in `content.test.ts` and pass.
- [ ] The PR description has a table of each tag with its word count and the decks it draws from.
- [ ] `npm test` and `npm run build` pass.

## Tags Phase 2: Custom study page

- `src/study/tags.ts` + `tags.test.ts`, as described in Design: `tagsOf`, `matchingUnits`, `pickUnits`, `customDeckFrom`, `buildCustomDeck`.
...
- [ ] `tags.test.ts` covers:
  ...
  - removing duplicate words across decks, keeping the earlier deck's copy
